...
export class PermissionService extends BaseService {
    ...
    save(roleId: number, request: PermissionUpdateRequest) {
        return this.http.post<ResponseData>(`/api/permissions/InsertWithRoleId/${roleId}`, request,
    ...
    getPermissionsByRole(roleId: string) {
        return this.http
        .get<ResponseData>(`/api/permissions/SelectByRoleId/${roleId}`, { headers: this.httpOptions })
        .pipe(catchError(this.handleError));
    }
}
